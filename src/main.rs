...
const NUM_TRADER_THREADS: usize =
    NUM_PERSONAL_THREADS + NUM_FUNDAMENTAL_THREADS + NUM_NOISE_THREADS;
...
fn main() {
    ...
    let mut order_book = Book::new(
        highest_ask_index.clone(),
        lowest_ask_index.clone(),
        highest_bid_index.clone(),
        lowest_bid_index.clone(),
    );
